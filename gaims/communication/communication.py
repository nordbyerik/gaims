...
class CommunicationMedium(ABC):
    ...
    def __init__(self, adjacency_list: Dict[str, List[str]], agent_names: List[str]):
    ...
    def send_message(
        self, sender: str, receiver: str | List[str], message_content: str
    ) -> Message:
        """
        ...
        if receiver is None or receiver == "None":
            self.broadcast_message(sender, message_content)
            return
        ...
        self.message_history.append((sender, receiver, msg_obj))
        return msg_obj
    ...
    def get_all_pending_messages_for_agent(
        self, agent_name: str
    ...
    def get_communication_partners(self, agent_name: str) -> List[str]:
        possible_partners = self.adjacency_list.get(agent_name, [])
        possible_partners = [self.agent_names[x] for x in possible_partners]
...
class FullyConnectedCommunicationMedium(CommunicationMedium):
    ...
    def __init__(self, agents: List[str]):
        ...
        super().__init__(adjacency_list, agent_names)
        ...
        if not agents:
            ...
            for agent in agents:
                ...
                if not possible_receivers:
                    ...
                    adjacency_list[agent] = random.sample(
                        possible_receivers, num_connections
                    )
        agent_names = [agent.name for agent in agents]
        super().__init__(adjacency_list, agent_names)
